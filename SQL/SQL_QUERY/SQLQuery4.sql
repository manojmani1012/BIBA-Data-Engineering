...
select distinct city ,count(*) as count_city from employee group by city;
update employee set salary = 45000 where id=104;
select count(salary) as count_salary, salary  from employee group by salary having  salary > 40000;
select count(salary) as count_salary , salary from employee group by salary order by salary asc;
begin tran;
...
/*string func-len,lower,upper,left,right,concat,ltrim,Rtrim,substring,replace,charindex*/
/*date func- getdate,sysdatetime,CURRENT_TIMESTAMP,DATEPART,dateadd,datediff,format,convert,monthand year,eomonth,GETUTCDATE,DATEDIFF and DATEADD */
/*mathematical func-arthimetic,round,log,abs,rand,sign,exp,sqrt,power,ceiling and floor*/
/*system func-DB_Name(),@@version,current_user,seesio_user,@@spid,user_name,serverproperty,@@rowcount*/
